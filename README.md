...
当前已进入一阶段代码开发，完成后端 FastAPI 服务、数据库迁移、Tushare SDK 同步服务、A/H 溢价计算、LLM 问答 API、React 前端页面和开发进度文档。Tushare 默认使用中转服务 `http://tsy.xiaodefa.cn`，Token 优先从本机文件读取，环境变量作为兜底。
