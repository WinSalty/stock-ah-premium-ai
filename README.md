...
本项目用于通过 Tushare 拉取 A 股、港股基础、港股通名单、官方 AH 比价、分红和汇率数据，持久化到本地 MySQL，并展示港股通标的中的 A/H、H/A 溢价与 A 股长期分红再投入筛选结果。项目提供前端页面查看同步状态、溢价榜单、分红再投榜单、趋势和本地数据查询，并通过 LLM API 对本地数据进行安全问答分析。

当前已进入一阶段代码开发，完成后端 FastAPI 服务、数据库迁移、Tushare SDK 同步服务、定时增量跑批、统一数据查询、A/H 溢价计算、LLM 问答 API、React 前端页面和开发进度文档。Tushare 默认使用中转服务 `https://tt.xiaodefa.cn`，Token 优先从本机文件读取，环境变量作为兜底。
...
- Tushare 默认通过 Python SDK 调用，接口地址为 `https://tt.xiaodefa.cn`；Tushare Token 优先使用本机未入库文件，LLM API Key、数据库账号密码通过环境变量或本机未入库配置注入。
