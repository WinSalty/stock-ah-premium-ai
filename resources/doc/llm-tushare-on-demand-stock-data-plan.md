...
用户当前只有 Tushare 15000 积分权限，这是本方案的硬边界。系统默认只做单股、短区间、低频、缓存优先的按需补齐，不允许 LLM 自动触发全市场、多股票、大长周期批量拉取。个股投资分析报告是第一优先级优化场景：我们给 LLM 足够完整的数据和分析方法，但不过度约束输出模板，让模型保留专业判断、推理链组织和表达自由度。
...
- 单轮问答默认只允许 1 只股票自动补数。
- 单轮问答最多触发 3 个数据包、6 次 Tushare API 请求。
...
当用户问题需要高成本数据，例如“比较所有银行谁最稳”“筛全市场现金流改善股票”“回测十年低估值策略”，系统应提示需要管理员预热、缩小范围或使用已有候选宽表，不由 LLM 自动拉取。
...
| --- | --- | --- | --- | --- |
| `stock_identity` | “这家公司代码是什么” | `stock_basic`、`hk_basic`、本地 `ah_stock_pair` | 单股解析 | 是 |
| `quote_valuation` | “现在贵不贵”“股价趋势如何” | `daily`、`daily_basic` | 单股最近 120 交易日 | 是 |
| `financial_statement` | “最新财报质量如何” | `income`、`balancesheet`、`cashflow`、`fina_indicator` | 单股 12 季度/5 年 | 是 |
| `dividend_forecast` | “分红稳定吗”“业绩预告怎样” | `dividend`、`forecast`、`express` | 单股最近 5 年/最近若干条 | 是 |
...
“谨慎”表示只在单股、短区间、用户问题明确且本地缺数时自动触发；否则提示缩小范围。
...
```json
{
  ...
  "data_demands": [
    {
      "intent": "single_stock_research_report",
      "stock_hints": ["招商银行", "600036.SH"],
      "market_scope": "A_STOCK",
      "data_packages": ["stock_identity", "quote_valuation", "financial_statement", "dividend_forecast"],
      "period_policy": "latest_12_quarters",
      "reason": "用户要求生成个股投资分析报告，需要基本面、估值和分红数据"
    }
  ],
  "reason": "问题属于单家公司投资分析，本地材料不足时可按需补齐低成本数据"
}
```

后端二次校验规则：

- 股票必须唯一解析；不能唯一解析时返回候选让用户澄清。
- `symbols` 超过 1 只时，不自动触发高成本数据包。
- `data_packages` 必须在白名单内。
- 所有数据包都要先检查本地缓存和只读视图。
- 超过 15000 积分保守策略的请求直接降级为“需要缩小范围或管理员预热”。
...
- 用户问“A/H 溢价机会是否有基本面支撑”，系统先查现有 AH 视图，再按单股补齐财务摘要，不重复拉全市场数据。
