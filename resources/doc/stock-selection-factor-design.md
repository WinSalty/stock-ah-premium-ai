...
当用户要求生成单家上市公司投资分析报告，或分析最新财报、估值、利润质量、资产负债、经营现金流、ROE 变化等问题时，不应扩大这张候选宽表的职责；应按 [LLM 按需补齐 Tushare 股票数据通用开发方案](./llm-tushare-on-demand-stock-data-plan.md) 走单股受控补数。该通用方案以 15000 积分权限为硬边界，只允许单股、短区间、低频、缓存优先的数据包自动补齐，再把已落库的摘要交给 LLM 自主组织专业投资分析。
