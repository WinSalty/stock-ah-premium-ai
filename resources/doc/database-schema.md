...
- `ai_image_generation`：AI 图片生成记录表，保存用户提示词、模型、尺寸、生成状态、参考图和输出图的本地相对路径、文件 hash、供应商脱敏请求/响应摘要和用户侧失败摘要；参考图上传后会先压缩并转成 JPEG，表内参考图 MIME、大小和 SHA256 均以压缩后内容为准。创建任务后先返回 `GENERATING`，后台任务继续生成，用户关闭页面后仍可通过历史记录查看后续状态。
- `ai_image_generation_error_log`：AI 图片生成错误日志表，保存后台任务失败阶段、异常类型、HTTP 状态码、重试次数和截断后的详细错误；仅供管理员排查，不直接展示给普通用户。
