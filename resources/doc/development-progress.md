...
- 前端 React 项目：
  ...
  - 智能问答页：会话列表、历史加载、逻辑删除、东八区时间显示、纯问题输入、基于投研报告随机展示的预设问题、流式回答和数据摘要表格；页面已重构为投研工作台布局，消息区独立滚动，回答表格和数据摘要不再撑破页面；新增最新消息自动滚动锚点并优化问答间距，生成中状态改用真实加载组件，避免伪进度动效和问题气泡视觉遮盖。
