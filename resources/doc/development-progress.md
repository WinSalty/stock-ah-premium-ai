...
- 前端 React 项目：
  ...
  - 总览页自选机会卡片始终通过 `/api/watchlist` 读取官方 AH 比价主表口径；交易时段前端会后台触发 `/api/ah-premiums/realtime?only_watchlist=true`，后端用实时快照计算后写回 `official_ah_comparison` 并标记 `is_realtime`，再刷新自选卡片数据。实时写回时严格区分数据日期和生成时间：`trade_date` 使用 A/H 报价自身的 `quote_time` 推导，`source_updated_at`/`updated_at` 才表示系统生成或写入时间。卡片右下角不再展示更新时间。
