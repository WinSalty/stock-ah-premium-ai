...
- 图片生成供应商 `input-images per min` 限流短重试上限已从 10 次调整为 30 次，仅对明确包含该限流文本的响应生效，其他供应商错误仍不进入该重试路径。
- 图片生成参考图已改为上传后后端统一压缩并转成 JPEG，OSS `references/` 目录只保存压缩图，供应商调用和失败重试均复用压缩图；图片生成默认超时从 300 秒放宽到 500 秒，`ReadTimeout` 不重复请求，其他短暂 HTTP/网络错误在原有限流重试之外最多额外重试 5 次并记录总 `retry_count`。
...
- 移动端底部菜单稳定性已修复：移动端应用壳改为固定在真实可视区域内，并通过 `visualViewport` 同步 `--mobile-app-height`，避免浏览器地址栏收缩、页面惯性滚动或 WebView 高度变化时把底部导航推到页面中部；底部导航设置为不参与 flex 压缩的全局入口。`npm --prefix frontend run build` 已通过，并用本地 mock API 和 in-app Browser 验证 390x844 视口下问答页、机会筛选页底部菜单均贴底，页面切换后内容滚动位置归零。
...
- LLM 问答：
  ...
  - 问答流式接口已改为后台线程继续执行和独立数据库会话落库，浏览器离开页面或断开连接后仍会跑完 LLM 任务并保存 assistant 消息；移动端键盘弹起时保留应用壳稳定高度，只压缩问答工作区并隐藏底部导航，避免输入时整页被顶开。
  ...
  - LLM 问答已接入分红再投筛选结果：前置路由新增 `dividend_reinvestment` 模式，遇到近十年平均年化、ROE、PE、股息率、连续分红、分红再投筛选或单股年度明细问题时，直接把 `dividend_reinvestment_backtest_run`、`dividend_reinvestment_backtest_summary` 和 `dividend_reinvestment_backtest_yearly` 的表结构字段与用途喂给 SQL 生成阶段，由模型生成只读 SQL 回查最新完成批次；“招商银行 年度明细”这类短追问会跳过普通追问快路径并继续回表，回答阶段补充分红再投 Markdown 表格示例以稳定 GFM 输出。
  - LLM 问答新增投资偏好推荐路由：用户问“我可以投资哪些股票”“我该买什么股票”等泛投资推荐入口时，先本地反问长期持有保守型还是风险高收益型，反问文案只描述投资风格，不暴露内部数据来源；用户选择保守型后进入 `conservative_value_recommendation`，由 SQL 模型基于分红再投三张表字段自行回查不超过 10 只价值投资候选并生成配置策略；用户选择风险高收益型后进入 `aggressive_limit_up_recommendation`，读取 `limit_up_analysis_cache` 最新 READY 打板推送报告，由回答模型挑选更有望晋级观察的短线标的并显著提示风险。问答接口和前端已不再向用户返回或展示 rows 数据摘要，rows 仅保留在 assistant 消息落库预览中用于服务端审计。
  ...
  - 问答 API 和前端均不再向用户展示 SQL 原文，也不再展示底层 rows 数据摘要；回答仍可把必要数据整理成 Markdown 正文。
- 统一数据查询：
...
- 前端 React 项目：
  ...
  - 数据同步页：已改为先按业务目标选择同步方案，推荐入口拆成“A/H 主链路”和“分红再投”两张操作卡，卡片内直接说明会刷新哪些业务页面和数据范围；单数据集同步收敛为高级补数区，人工导入独立保留，任务记录仍支持筛选和错误详情查看。
  - 数据同步页分红再投卡片新增“仅本地回测：不访问 Tushare”模式，用于原始数据已落库后只重算分红再投榜单和年度明细，并自动禁用逐股分红、ROE 补数开关。
  - 数据同步页任务记录的“任务说明”会结合数据集、同步模式、日期范围、分红再投阶段结果生成可读说明；参数、错误和说明等长字段支持悬浮查看完整内容。
  ...
  - 智能问答的问数模式 Markdown 表头已扩充为中文字段名，覆盖行情估值、财务摘要、利润表、现金流量表、资产负债表、分红预告和 A/H 溢价字段；底层 rows 数据摘要已不再返回或展示，必要数据由 LLM 整理进 Markdown 正文。
  - 智能问答页预设问题按钮改为点击后直接发送，仍复用输入框提交的同一套流式问答逻辑；新对话示例池已补充分红再投筛选、年度明细、泛投资推荐、长期持有保守型推荐和风险高收益型打板观察等新路由问题。
