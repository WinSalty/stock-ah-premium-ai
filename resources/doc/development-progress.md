...
- A/H 溢价计算：
  ...
  - 官方 AH 查询新增港股通通道、自选状态、关注方向、目标阈值距离、20/60/120 日均值、60 日分位和偏离 60 日均值字段。
- 自选股能力：
...
- 前端 React 项目：
  ...
  - 总览页趋势图支持日期范围缩放、阈值线、20 日均值线和 60 日均值线；无自选时保留全市场趋势兜底。
  ...
  - AH 溢价页：基于官方 AH 比价表展示，支持港股通/自选/通道/AH 与 H/A 区间筛选、加入自选时维护目标阈值、编辑自选、取消自选、派生指标重算、趋势抽屉和公式悬浮提示。
...
2. 启动前后端后添加几只自选股票，验证首页机会卡片、阈值距离、通道、趋势均值线和溢价页加入自选流程。
