...
- 消息推送与提醒：
  ...
  - PushPlus 绑定功能保留在个人信息页；已绑定用户不再展示二维码绑定入口，也不能重复生成二维码或覆盖绑定；管理员的好友列表、用户绑定信息管理和“系统用户 + PushPlus 好友”手动绑定已移入用户管理菜单，绑定时会把好友令牌仅保存到后端。
  - PushPlus 测试消息、阈值提醒和股价提醒统一使用 HTML 模板发送，消息内展示触发类型、标的、交易日、当前阈值/价格和目标阈值/价格等明细。
...
- LLM 问答：
  ...
  - 已将 `llm-knowledge/公司价值投资分析报告_2026版/` 中的中国神华、格力电器、宁德时代、比亚迪、长江电力和寒武纪 2026 版公司价值投资报告登记到个股深度投资报告分类，按公司名、股票代码和价值投资关键词命中。
