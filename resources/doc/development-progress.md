...
- LLM 问答：
  ...
  - LLM 问答新增投资偏好推荐路由：用户问“我可以投资哪些股票”“我该买什么股票”等泛投资推荐入口时，先本地反问长期持有保守型还是风险高收益型；用户选择保守型后进入 `conservative_value_recommendation`，由 SQL 模型基于分红再投三张表字段自行回查不超过 10 只价值投资候选并生成配置策略；用户选择风险高收益型后进入 `aggressive_limit_up_recommendation`，读取 `limit_up_analysis_cache` 最新 READY 打板推送报告，由回答模型挑选更有望晋级观察的短线标的并显著提示风险。问答接口和前端已不再向用户返回或展示 rows 数据摘要，rows 仅保留在 assistant 消息落库预览中用于服务端审计。
