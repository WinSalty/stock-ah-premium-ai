...
- LLM 问答：
  ...
  - 新增 LLM 专用投资知识库 `resources/doc/llm-knowledge/`，按 A/H 跨市场价差、A 股选股估值、银行与非银、宏观产业推演、组合风险与报告框架分组；问答时按问题和上下文选择性读取 Markdown 与 DOCX 片段。
...
- 总览页和 AH 机会筛选页的“AI 推荐阈值”入口新增结构化 `threshold_recommendation` 上下文，只传页面已有的 A/H 代码、关注方向、持有侧、当前阈值、当前价差、60 日中位数、20/80 分位、当前分位和港股通通道；后端据此识别固定阈值场景，不再进入通用问题路由、股票消歧、Tushare 按需补数、知识库材料注入和 `v_watchlist_opportunity` 辅助查询。
