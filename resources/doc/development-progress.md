...
- 新增雪球发布管理模块：后端新增 `xueqiu_publish_credential` 和 `xueqiu_publish_record`，用于本地保存雪球创作者后台 Cookie 登录态摘要、草稿/发布流水、请求摘要和雪球响应；新增 `xueqiu_publish` 菜单权限并默认授予管理员，普通用户不自动获得入口。新增 `/api/xueqiu-publish/...` 管理接口，支持保存登录态、验证登录态、预览最新 READY 打板报告转换后的雪球长文、保存草稿、正式发布和查看流水详情。新增 `XUEQIU_PUBLISH_SCHEDULER_ENABLED=false` 和 `XUEQIU_PUBLISH_AUTO_PUBLISH=false` 配置，默认不启用定时、不自动公开发布；若后续显式启用，早盘任务也会通过 `analysis_id + publish_mode` 幂等避免重复发文。前端新增“雪球发布”管理员菜单，提供登录态配置、报告预览、保存草稿、正式发布确认和流水查看；手动勾选“强制新建/重试”时会清空本地旧草稿 ID，适配管理员已在雪球网页端删除草稿后重新创建草稿的场景。雪球请求头按正常浏览器同源请求补齐 `Origin`、`Referer`、`User-Agent`、`X-Requested-With` 等字段；遇到验证码、风控或接口变更时记录失败并交由人工处理，不保存账号密码。
