...
## 试用反馈修复·第三轮（2026-06-12）

- **个股分析灵活出图**：提示词 v3 把 11 条方法论定位为"分析口径而非输出模板"，个股分析建议出 1~3 张关键趋势图、图文结合、图表不与表格重复表达；新增按数据特征选图型的出图策略（趋势 line/双量纲 dual_axis、对比 bar、占比 pie、分布 scatter、行情 kline）。实测"分析一下长江电力"：财务包为主材料 + 3 张图嵌入正文 + 营收利润/盈利质量/现金流/资产负债结构化分析。
- **轮内取数上限 8→16**（用户要求 16）：个股全景分析+出图需多视图取数，本地取数成本低，成本安全网由 LLM 日限额与迭代上限承担。
- **图表趋势不明显修复**：ChatChart 数值轴 `scale: true` 按数据范围缩放（此前汇率 6.8 在 0 起点轴上是平线）；bar 保持 0 起点不误导；单系列折线加渐变面积；dual_axis 双轴同步缩放。预览实测 -6.4%~-7.x% 区间铺满纵轴、波动清晰。
...
- **真流式恢复 + 整轮耗时修正（同日追加）**：①迭代调用改为带 tools 的流式（`llm_client.stream_chat_with_tools`：content 增量实时透传、tool_calls 按 OpenAI 协议增量聚合、头部 48 字符泄漏护栏）——模型直接作答时 token 级渐进渲染，决定调工具时聚合执行，单次调用兼顾打字机体验与工具解析，且无"二次生成改判调工具"的泄漏土壤；本地实测 375 个增量跨 6.2 秒渐进到达。②done 事件新增 `elapsed_ms` 整轮墙钟（含模型思考），前端折叠摘要改显"总耗时 Xs（含模型分析）"，替代仅加总工具执行时间的误导口径（一轮 60s 对话此前显示 1.1s）；历史回放无墙钟值时只显示步数；毫秒级单步显示 "<0.1s"。
