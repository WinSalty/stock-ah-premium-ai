...
- 所有打板专题接口返回的股票行统一过滤 ST、*ST、S*ST 等风险警示股票，过滤数量写入 `data_quality`，被过滤股票不进入题材、连板、龙虎榜和技术指标上下文。
...
- 当前项目账号暂未开通 `stk_nineturn`，神奇九转自动调度、手动生成、手动推送和雪球发布入口先软关闭；数据模型、服务实现和历史查询页面保留，待权限开通后再恢复入口。
...
- 对技术指标股票池做上限控制，例如最多 240 只，优先二连、三连、高标和题材代表股。
...
- 强制两连板、三连板分别输出表格，并在各自表格后做重点分析；两连板看晋级三板条件，三连板看空间地位、分歧承接和断板风险；首板可按题材发酵价值简述，不强制输出表格。
...
- 最新报告：展示交易日、生成时间、状态、模型、数据质量，完整报告支持渲染预览和原始源码两种查看方式。
- 历史报告：按交易日、状态搜索。
...
- 已新增并落地多阶段分析改造 `20260605_0047_add_limit_up_multi_stage_cache.py`：报告生成拆为首板题材发酵、两连三连候选筛选、高连板候选筛选、筹码补数、重点分析和最终合成；两连三连重点候选默认最多 20 只，高连板默认最多 10 只，入选股票按需调用 `cyq_perf` 与 `cyq_chips` 后再分析连板和下一个交易日溢价可能性。后台报告详情页新增“阶段”页签，可核对阶段质量、两连三连入选名单和高连板入选名单。
