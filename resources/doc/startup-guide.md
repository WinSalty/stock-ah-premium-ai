...
如果原始行情、分红、估值和 ROE 已经落库，只需要重新生成分红再投筛选结果，可在“同步分红再投数据”中选择 `仅本地回测：不访问 Tushare`。该模式只重算 `dividend_reinvestment_backtest_run`、`dividend_reinvestment_backtest_summary` 和 `dividend_reinvestment_backtest_yearly`，不会重新请求或删除 `a_daily_quote`、`a_dividend`、`a_daily_basic`、`a_financial_indicator` 等原始数据表。
