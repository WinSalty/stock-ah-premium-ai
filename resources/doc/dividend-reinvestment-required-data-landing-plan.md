...
| 参数 | 使用场景 |
| --- | --- |
| `ts_code` | 候选股票历史分红补洞或人工复核；实测可补齐部分 `ex_date` 全市场日期回补缺失的早期实施记录。 |
...
- 修复或全量校准时可显式开启 `supplement_dividend_by_stock`，再按候选股票逐只用 `ts_code` 补齐历史记录；该动作不作为每日增量默认项，避免日常同步产生数千次请求。
