...
- 处理日期：2026-06-11
- 处理范围：第五节追加评审中的 P1-1、P2-1、P2-2、P2-3 与部分 Nit。

### 已处理

- P1-1：`GENERATING` 僵死判断改为应用侧 UTC-naive 口径。新建报告和重跑重置时主动写入 `updated_at = _now_naive()`，`_is_generating_stale` 也使用 `_now_naive()` 比较，避免依赖数据库服务器时区。
- P2-1：`_focus_ts_codes` 改为复用 `_board_level(row) >= 1`，保证 `N天M板` 股票进入技术指标补数范围。
- P2-2：炸板率改为按 `ts_code` 去重，分母为涨停池与炸板池代码并集，分子为剔除已回封涨停代码后的炸板代码数，并在 `emotion_cycle` 中输出 `broken_board_rate_scope` 说明口径。
- P2-3：报告列表/详情新增 `has_stage_fallback`，当 `pipeline.stage_quality` 中存在 `FAILED_FALLBACK` 时标记为 true，方便管理员决定是否手动重跑。
- Nit：`quote_sample_count` 改为实际有 `pct_chg` 的样本数，并新增 `high_open_sample_count`；高连板筛选 prompt 补齐 `score_detail` 评分维度。
- Prompt 缓存：高连板筛选 schema 发生变化，阶段缓存版本后缀从 `:v2` 升为 `:v3`。

### 验证

```bash
cd /Users/salty/codeProject/ai/coding/stock-ah-premium-ai/backend && .venv/bin/python -m py_compile app/services/limit_up_push_service.py app/schemas/limit_up_push.py tests/test_limit_up_push_service.py
cd /Users/salty/codeProject/ai/coding/stock-ah-premium-ai/backend && .venv/bin/pytest tests/test_limit_up_push_service.py
```
...
结果：`242 passed, 2 failed`。失败用例仍为第三节记录过的 `tests/test_auth_service.py::test_profile_update_only_changes_basic_fields` 与 `tests/test_market_data_orchestrator.py::test_orchestrator_uses_cache_for_recent_quote_package`，未新增打板模块失败。
