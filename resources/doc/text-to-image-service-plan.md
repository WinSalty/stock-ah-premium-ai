...
- 对明确包含 `input-images per min` 的图片输入限流响应做最多 10 次短重试；若仍失败，详细错误写入 `ai_image_generation_error_log`，普通用户只看到友好失败摘要。
...
- `input-images per min` 图片输入限流响应最多重试 10 次；重试后仍失败时记录 `retry_count`。
- 管理员修改每日上限、重置今日次数生效。
