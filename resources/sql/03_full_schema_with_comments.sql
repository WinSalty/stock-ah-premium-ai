...
CREATE TABLE IF NOT EXISTS `ai_image_generation` (
  ...
  `file_relative_path` VARCHAR(512) DEFAULT NULL COMMENT '输出图片存储对象键，OSS 模式为 Bucket 内 object key',
  `file_size_bytes` INT DEFAULT NULL COMMENT '输出图片字节数',
  `file_sha256` VARCHAR(64) DEFAULT NULL COMMENT '输出图片 SHA256',
  `reference_file_relative_path` VARCHAR(512) DEFAULT NULL COMMENT '参考图存储对象键，OSS 模式为 Bucket 内 object key',
  ...
  `elapsed_ms` DOUBLE DEFAULT NULL COMMENT '生成、下载和保存到存储后端的总耗时毫秒',
...
CREATE TABLE IF NOT EXISTS `limit_up_analysis_cache` (
  ...
  `advice_status` VARCHAR(16) NOT NULL DEFAULT 'PENDING' COMMENT '投资建议状态：PENDING/GENERATING/READY/FAILED',
  `advice_html` LONGTEXT DEFAULT NULL COMMENT '投资建议正文 HTML（规整后，PushPlus/雪球/详情页共用）',
  `advice_markdown` LONGTEXT DEFAULT NULL COMMENT '投资建议 LLM 原始输出（对齐 content_markdown 双存惯例）',
  `advice_generated_at` DATETIME DEFAULT NULL COMMENT '投资建议生成时间（UTC-naive，与报告 generated_at 同口径）',
  `advice_error` TEXT DEFAULT NULL COMMENT '投资建议生成失败原因（截断 1000 字）',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `limit_up_analysis_stage_cache` (
  ...
  `stage_key` VARCHAR(64) NOT NULL COMMENT '阶段标识，FIRST_BOARD、FIRST_BOARD_SELECTION、CHAIN_SELECTION、HIGH_BOARD_SELECTION、FIRST_BOARD_FOCUS、CHAIN_FOCUS、HIGH_BOARD_FOCUS、FINAL_REPORT、INVESTMENT_ADVICE',
...
CREATE TABLE IF NOT EXISTS `limit_up_selected_stock` (
  ...
  `sentiment_cycle` VARCHAR(16) DEFAULT NULL COMMENT '六档情绪周期：冰点/退潮/分歧/启动/发酵/高潮',
  `market_state` VARCHAR(16) DEFAULT NULL COMMENT '三档可参与状态(由sentiment_cycle映射)：空仓/谨慎参与/参与',
