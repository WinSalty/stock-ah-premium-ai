...
// 预设问题按 Agent 引擎当前能力组织（2026-06-13 更新）：
// A/H 溢价与自选阈值、个股研究（财务方法论）、图表展示（render_chart）、
// 沙箱计算（run_python）、联网搜索（web_search）、分红再投与打板推荐、分析框架。
// 空会话时随机抽 PRESET_QUESTION_COUNT 条展示，点击即发送，要求每条都能直接执行成功。
const STRUCTURED_ANALYSIS_PRESET_QUESTIONS = [
  // —— A/H 溢价与自选阈值（核心业务）——
  '我关注的股票里，哪些 A/H 价差最接近目标阈值？按优先级排个序',
  '当前全市场 H 股折价最深的港股通标的有哪些？',
  '招商银行现在更适合持有 A 股还是 H 股？给出行动条件和反证条件',
  '我关注的股票里，哪些 A/H 价差更像择边机会而不是套利机会？',
  // —— 图表展示 ——
  '画一张招商银行近 60 日 A/H 溢价走势图，并解读趋势',
  '用图表对比工商银行、建设银行、招商银行最新的 ROE、PB 和股息率',
  '用图表展示宁德时代近五年营收和归母净利润趋势，并点评增长质量',
  '招商银行分红再投年度明细是什么样？用图表展示逐年收益',
  // —— 沙箱计算 ——
  '计算招商银行和建设银行近 60 个交易日收盘价的相关性和各自波动率',
  '假设过去一年 H/A 溢价每超过 8% 就换仓到 H 股，回测招商银行这个策略的收益',
  '招商银行十年平均年化收益率是多少？',
  // —— 联网搜索（时效资讯）——
  '最近有什么影响 A 股银行板块的重要政策或新闻？',
  '美联储最近一次议息会议的结果对港股有什么影响？',
  '搜索一下招商银行最近的公告和市场解读，结合本地数据给个判断',
  // —— 个股研究（财务方法论）——
