...
AGGRESSIVE_LIMIT_UP_RECOMMENDATION_MODE = "aggressive_limit_up_recommendation"
...
INVESTMENT_RECOMMENDATION_KEYWORDS = (
    "我可以投资哪些股票",
    "我能投资哪些股票",
    "我该买什么股票",
    "该买什么股票",
...
CONSERVATIVE_INVESTMENT_KEYWORDS = (
    "保守",
    "长期持有",
    "长期投资",
    "价值投资",
...
AGGRESSIVE_INVESTMENT_KEYWORDS = (
    "风险",
    "高收益",
    "高风险",
    "进取",
...
INVESTMENT_PREFERENCE_CLARIFICATION_MESSAGE = (
    "你更偏哪一种投资风格？\n\n"
    "- **长期持有保守型：** 我会结合分红再投筛选数据，挑不超过 10 只更适合价值投资"
    "和长期持有的股票，并给出配置策略。\n"
    "- **风险高收益型：** 我会结合最新打板推送报告，挑几只更有望晋级观察的短线标的，"
    "并把风险放在最前面说明。"
)
...
不要输出 data_demands。
- `investment_preference_clarification`：用户泛泛询问“我可以投资哪些股票”“我该买什么股票”
等推荐入口，但未说明风险偏好；这类必须先反问长期持有保守型还是风险高收益型，不要生成 SQL。
- `conservative_value_recommendation`：用户选择长期持有保守型或稳健价值投资；这类必须生成 SQL，
让 SQL 从分红再投筛选数据里回查不超过 10 只价值投资候选，不要输出 data_demands。
- `aggressive_limit_up_recommendation`：用户选择风险高收益型、短线、打板、连板或晋级观察；
这类必须生成 SQL，读取最新打板推送报告内容，不要输出 data_demands，并在最终回答显著提示风险。
...
后端会先让 LLM 从自然语言直接识别股票代码并做本地基础表验真，失败后再进入本地候选消歧。
A 股数据包含义：
...
如果用户问“我可以投资哪些股票”“我该买什么股票”“推荐股票”等泛投资推荐入口，
但没有说明风险偏好，必须返回 `answer_mode="investment_preference_clarification"`、
`needs_sql=false`、`data_demands=[]`。
如果上下文里上一轮已经要求用户选择投资风格，且当前用户回答“长期持有保守型/保守/稳健/价值投资”，
必须返回 `answer_mode="conservative_value_recommendation"`、`needs_sql=true`、`data_demands=[]`。
如果上下文里上一轮已经要求用户选择投资风格，且当前用户回答“风险高收益型/短线/打板/晋级观察”，
必须返回 `answer_mode="aggressive_limit_up_recommendation"`、`needs_sql=true`、`data_demands=[]`。
只返回 JSON，不要输出解释。格式：
{"is_answerable":true或false,"needs_sql":true或false,"answer_mode":"stock_research或full_report或open_research或data_only或dividend_reinvestment或investment_preference_clarification或conservative_value_recommendation或aggressive_limit_up_recommendation或general","data_demands":[{"market":"A或HK","ts_code":"600036.SH或02380.HK","packages":["quote_valuation","financial_statement","business_profile","dividend_forecast","shareholder_governance","capital_flow_light"],"intent":"stock_research"}],"reason":"一句话原因"}
"""
...
@dataclass(frozen=True)
class StockCodeExtractionResult:
    """LLM 直接从自然语言中识别股票代码后的结构化结果。
...
class LlmService:
    ...
    def answer(
        ...
        if route.answer_mode == INVESTMENT_PREFERENCE_CLARIFICATION_MODE:
            # 泛投资推荐入口先收集风险偏好，不消耗回答模型调用；
            # 后续“保守型/风险高收益型”短追问会被本地路由识别为新的数据任务。
            self._log_total_elapsed(
                "sync_investment_preference_clarification",
                question_id,
                selected_model,
                started_at,
                user_id=user_id,
            ...
            return ChatAnswer(
                answer=INVESTMENT_PREFERENCE_CLARIFICATION_MESSAGE,
                sql=None,
                rows=[],
            )
    ...
    def stream_answer(
        ...
        if route.answer_mode == INVESTMENT_PREFERENCE_CLARIFICATION_MODE:
            # 流式接口同样先给固定偏好澄清，避免未选择风险偏好时提前推荐股票。
            self._log_total_elapsed(
                "stream_investment_preference_clarification",
                question_id,
                selected_model,
                started_at,
                user_id=user_id,
            ...
            return None, [], iter([INVESTMENT_PREFERENCE_CLARIFICATION_MESSAGE])
        endpoint = self._model_endpoint(selected_model)
        if not endpoint.api_key or not endpoint.model:
            message = (
                f"{endpoint.provider} LLM 未配置。请设置对应 API Key 文件或环境变量，"
    ...
    def _prepare_answer(
        ...
        if should_run_sql:
            try:
                sql = (
                    self._default_sql_for_route(route)
                    or self._default_sql_for_question(question, context)
                    or self._generate_sql(
                        question,
                        context,
                        model,
                        question_id,
    ...
    def _is_follow_up_question(self, question: str, context: dict[str, Any]) -> bool:
        ...
        if self._is_dividend_reinvestment_question(question):
            ...
            return False
        if self._investment_recommendation_route(question, context) is not None:
            # “保守型/风险高收益型”这类偏好回复必须进入新的数据路由；
            # 如果被追问快路径截走，就无法读取分红再投表或最新打板报告。
            return False
        try:
    ...
    def _answer_style_policy(
        ...
        if answer_mode == CONSERVATIVE_VALUE_RECOMMENDATION_MODE:
            return (
                "这是长期持有保守型股票推荐场景。回答必须结合分红再投筛选结果，"
                "推荐不超过 10 只更适合价值投资和长期持有的股票；"
                "先给组合结论，再说明每只股票的红利复利、估值、ROE、分红稳定性和数据质量观察，"
                "最后给分批配置、跟踪和退出策略。表格必须使用合法 GFM 格式。"
            )
        if answer_mode == AGGRESSIVE_LIMIT_UP_RECOMMENDATION_MODE:
            return (
                "这是风险高收益型短线观察场景。回答必须基于最新打板推送报告内容，"
                "挑几只最有望晋级观察的股票；第一块要用醒目标题显著提示高波动、回撤、"
                "晋级失败和隔日流动性风险，再说明观察理由、触发条件、仓位纪律和放弃条件。"
                "不要把短线观察写成长期价值投资建议。"
            )
    ...
    def _sql_prompt(self, question: str, context: dict[str, Any]) -> str:
        ...
        return (
            ...
            "涉及长期持有保守型、稳健价值投资或红利复利推荐时，"
            "优先让 SQL 从分红再投三张表回查最新完成批次，结合长期年化、分红稳定性、估值、ROE、"
            "数据质量等字段自行筛选，不超过 10 只候选。"
            "涉及风险高收益型、短线打板、连板晋级观察时，"
            "查询 limit_up_analysis_cache 最新 status='READY' 的报告正文，"
            "优先返回 trade_date、title、content_markdown、content_html、generated_at。"
    ...
    def _normalized_question_text(self, question: str) -> str:
        """统一投资偏好路由的文本口径，避免空格、斜杠和大小写导致漏判。
    ...
    def _is_broad_investment_recommendation_question(self, question: str) -> bool:
        """识别没有限定标的的泛投资推荐入口。
        ...
        normalized = self._normalized_question_text(question)
        return any(keyword in normalized for keyword in INVESTMENT_RECOMMENDATION_KEYWORDS)

    def _investment_preference_from_question(self, question: str) -> str | None:
        """从用户原文提取投资偏好，保守词优先处理“低风险”等组合词。
        ...
        normalized = self._normalized_question_text(question)
        if any(keyword in normalized for keyword in CONSERVATIVE_INVESTMENT_KEYWORDS):
            return "conservative"
        if any(keyword in normalized for keyword in AGGRESSIVE_INVESTMENT_KEYWORDS):
            return "aggressive"
        return None

    def _is_investment_preference_reply(self, question: str, context: dict[str, Any]) -> bool:
        """判断短消息是否是在回应上一轮投资风格反问。

        创建日期：2026-06-03
        author: codex
        """

        if self._investment_preference_from_question(question) is None:
            return False
        history_text = "\n".join(
            str(item.get("content") or "") for item in self._conversation_history(context)[-4:]
        )
        normalized_history = self._normalized_question_text(history_text)
        return (
            "长期持有保守型" in normalized_history
            and "风险高收益型" in normalized_history
        ) or any(
            keyword in normalized_history for keyword in INVESTMENT_RECOMMENDATION_KEYWORDS
        )

    def _investment_recommendation_route(
        self,
        question: str,
        context: dict[str, Any],
    ) -> QuestionRoute | None:
        ...
        is_broad_entry = self._is_broad_investment_recommendation_question(question)
        is_preference_reply = self._is_investment_preference_reply(question, context)
        if not is_broad_entry and not is_preference_reply:
            return None
        preference = self._investment_preference_from_question(question)
        if preference == "conservative":
            return QuestionRoute(
                is_answerable=True,
                should_query_data=True,
                data_demands=(),
                reason="用户选择长期持有保守型，进入分红再投价值推荐路由",
        ...
        if preference == "aggressive":
            return QuestionRoute(
                ...
                should_query_data=True,
                data_demands=(),
                reason="用户选择风险高收益型，进入最新打板报告推荐路由",
        ...
        return QuestionRoute(
            ...
            data_demands=(),
            reason="泛投资推荐入口需要先确认风险偏好",
            answer_mode=INVESTMENT_PREFERENCE_CLARIFICATION_MODE,
        )

    def _default_sql_for_route(self, route: QuestionRoute) -> str | None:
        """为必须落到固定数据源的路由提供兜底 SQL。

        创建日期：2026-06-03
        author: codex
        """

        if route.answer_mode != AGGRESSIVE_LIMIT_UP_RECOMMENDATION_MODE:
            return None
        # 风险高收益型推荐以最新 READY 打板报告为唯一默认数据源；
        # 回复阶段再由 LLM 从报告正文中挑选观察标的并显著提示高波动风险。
        return (
            "SELECT trade_date,title,content_markdown,content_html,generated_at "
            "FROM limit_up_analysis_cache "
            "WHERE status = 'READY' "
    ...
    def _route_question(
        ...
        context = context or {}
        investment_route = self._investment_recommendation_route(question, context)
        if investment_route is not None:
    ...
    def _route_with_semantic_stocks(
        ...
        if route.data_demands:
            return route
        if not self._route_needs_stock_identification(route):
            return route
        selected_ts_codes = self._direct_stock_ts_codes(
            question,
            context,
            trace=LlmCallTrace(
                question_id=question_id or self._new_trace_id(),
                phase="stock_code_extraction",
                user_id=user_id,
                session_id=session_id,
                conversation_title=self._metric_conversation_title(question_id),
                user_name=self._metric_user_name(question_id),
        ...
        demands = tuple(
            MarketDataDemand(
                ts_code=ts_code,
                packages=self._packages_for_semantic_stock(question, ts_code),
    ...
    def _route_needs_stock_identification(self, route: QuestionRoute) -> bool:
        """判断当前路由是否值得追加股票代码识别。
        ...
        # 只有模型已经判断问题需要结构化数据或属于个股研究时才追加识别，
        # 避免普通问候、开放闲聊和能力介绍多打一层外部模型调用。
        return route.should_query_data or route.answer_mode in {"stock_research", "full_report"}
    ...
    def _direct_stock_ts_codes(
        self,
        question: str,
        context: dict[str, Any],
        trace: LlmCallTrace | None,
    ) -> tuple[str, ...]:
        """优先让 LLM 直接从自然语言中提取股票代码，并用本地基础表验真。
        ...
        try:
            extracted = self._extract_stock_codes_from_question(question, context, trace)
        except (ValueError, KeyError, TypeError, json.JSONDecodeError, httpx.HTTPError):
            # 直接识别只是优先路径；失败后回落候选消歧和本地确定性规则，
            # 不能让一次格式异常阻断整轮问答。
            logger.error("股票代码直接识别失败，回落候选消歧", exc_info=True)
            return ()
        if extracted.ambiguous or extracted.confidence < 0.85:
            return ()
        return extracted.selected_ts_codes

    def _extract_stock_codes_from_question(
        self,
        question: str,
        context: dict[str, Any],
        trace: LlmCallTrace | None,
    ) -> StockCodeExtractionResult:
        """调用 LLM 从用户原文直接抽取 A 股或港股代码。
        ...
        system_prompt = (
            "你是中国股票名称和代码识别器。用户可能输入 A 股、港股、股票简称、"
            "模糊简称或多股比较。不要依赖候选列表，直接从自然语言中识别股票。"
            "只返回 JSON 对象，不要解释过程。格式："
            '{"items":[{"name":"股票名","ts_code":"600036.SH 或 02380.HK",'
            '"market":"A 或 HK","confidence":0.0到1.0}],'
            '"ambiguous":true或false,"reason":"一句话"}。'
            "如果只是“平安”等明显歧义简称，必须 ambiguous=true；"
            "如果港股代码少于 5 位，也先按你知道的代码返回，后端会补齐校验。"
        )
        content = self._chat_completion(
            json.dumps(payload, ensure_ascii=False, default=str),
        ...
        for item in raw_items[:MAX_MARKET_DATA_STOCKS]:
            if not isinstance(item, dict):
                continue
            ...
            if confidence < 0.85:
                continue
            normalized_code = self._normalize_llm_stock_code(item.get("ts_code"))
            if normalized_code is None:
                continue
            # 所有 LLM 识别出的代码都必须二次命中本地基础表；命中后采用本地标准 ts_code，
            # 这样可自动纠正 market=SH/SZ 或 2380.HK 这类模型口径差异。
            stock_result = resolver.resolve_code(normalized_code)
            if not stock_result.resolved or stock_result.identity is None:
                continue
            if stock_result.identity.ts_code in selected_codes:
                continue
            selected_codes.append(stock_result.identity.ts_code)
            confidences.append(float(confidence))
        return StockCodeExtractionResult(
            selected_ts_codes=tuple(selected_codes[:MAX_MARKET_DATA_STOCKS]),
            reason=str(parsed.get("reason") or ""),
            confidence=min(confidences) if confidences else 0.0,
            ambiguous=ambiguous,
        )

    def _normalize_llm_stock_code(self, value: Any) -> str | None:
        """规范化 LLM 返回的 A 股或港股代码，供本地基础表验真。
        ...
        hk_match = re.fullmatch(r"(?P<symbol>\d{1,5})\.HK", code)
        if hk_match:
            # DeepSeek 可能返回 2380.HK；港股基础表使用 5 位代码，统一补零后再验真。
            return f"{int(hk_match.group('symbol')):05d}.HK"
    ...
    def _packages_for_semantic_stock(
        self,
        question: str,
        ...
        normalized = question.lower()
        if (ts_code and ts_code.endswith(".HK")) or re.search(r"\d{5}\.hk", normalized):
    ...
    def _local_question_route(
        ...
        investment_route = self._investment_recommendation_route(question, context or {})
        if investment_route is not None:
            return investment_route
    ...
    def _should_query_data(self, question: str, context: dict[str, Any]) -> bool:
        ...
        investment_route = self._investment_recommendation_route(question, context)
        if investment_route is not None:
            return investment_route.should_query_data
