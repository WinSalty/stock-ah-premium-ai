...
DATA_QUERY_SPECS: dict[str, QueryDatasetSpec] = {
    ...
    "stock_selection_factor_snapshot": QueryDatasetSpec(
        ...
        label="A 股选股因子",
        description="LLM 选股用核心因子宽表，覆盖蓝筹、低估值和红利候选股。",
        model=StockSelectionFactorSnapshot,
        columns=(
            col("factor_date", "因子日期", 132),
            col("ts_code", "代码", 130),
            col("name", "名称", 130),
            col("industry", "行业", 140),
            col("selection_tags", "标签", 170),
            col("selection_score", "综合分", 110),
            col("pe_ttm", "PE_TTM", 110),
            col("pb", "PB", 100),
            col("dividend_yield_ttm", "股息率", 110),
            col("roe", "ROE", 100),
            col("debt_to_assets", "资产负债率", 120),
            col("return_60d", "60日涨跌", 120),
            col("total_mv", "总市值(万元)", 150),
            col("selection_reason", "入选原因", 260),
        ),
        keyword_fields=("ts_code", "name", "industry", "selection_tags", "selection_reason"),
        date_field="factor_date",
        default_order=(("factor_date", "desc"), ("selection_score", "desc")),
    ),
    "dividend_reinvestment_backtest_summary": QueryDatasetSpec(
        name="dividend_reinvestment_backtest_summary",
        label="分红再投回测汇总",
        description="分红再投入筛选的股票级回测汇总结果，用于按年化收益、分红连续性和估值过滤候选股。",
        model=DividendReinvestmentBacktestSummary,
        columns=(
            col("run_id", "批次", 90),
            col("ts_code", "代码", 130),
            col("name", "名称", 130),
            col("industry", "行业", 140),
            col("start_trade_date", "开始交易日", 132),
            col("end_trade_date", "结束交易日", 132),
            col("initial_amount", "初始投入", 120),
            col("final_market_value", "期末市值", 120),
            col("total_cash_dividend", "累计分红", 120),
            col("total_reinvested_shares", "再投股数", 120),
            col("dividend_year_count", "分红年数", 100),
            col("consecutive_dividend_years", "连续分红年数", 130),
            col("total_return_pct", "总收益率", 110),
            col("annualized_return_pct", "年化收益率", 120),
            col("latest_dividend_yield_ttm", "最新股息率", 120),
            col("rank_score", "排序分", 110),
            col("data_quality", "数据质量", 110),
            col("data_issue", "数据问题", 220),
        ),
        keyword_fields=("ts_code", "symbol", "name", "industry", "data_quality", "data_issue"),
        date_field="end_trade_date",
        default_order=(("run_id", "desc"), ("rank_score", "desc")),
    ),
    "dividend_reinvestment_backtest_yearly": QueryDatasetSpec(
        name="dividend_reinvestment_backtest_yearly",
        label="分红再投年度明细",
        description="分红再投入回测的年度过程表，对应截图中的年度分红、再投入和收益字段。",
        model=DividendReinvestmentBacktestYearly,
        columns=(
            col("run_id", "批次", 90),
            col("ts_code", "代码", 130),
            col("year", "年份", 90),
            col("year_end_trade_date", "年末交易日", 132),
            col("year_end_price", "年末股价", 110),
            col("cash_div_per_share", "每股现金分红", 130),
            col("cash_div_amount", "分红金额", 120),
            col("stock_div_per_share", "送转比例", 110),
            col("reinvest_price_avg", "再投均价", 110),
            col("reinvested_shares", "再投股数", 120),
            col("holding_shares", "持仓股数", 120),
            col("market_value", "市值", 120),
            col("return_pct", "收益率", 110),
            col("annualized_return_pct", "年化收益率", 120),
            col("dividend_event_count", "分红次数", 100),
            col("note", "备注", 220),
        ),
        keyword_fields=("ts_code", "note"),
        date_field="year_end_trade_date",
        default_order=(("run_id", "desc"), ("ts_code", "asc"), ("year", "asc")),
    ),
    "dividend_reinvestment_backtest_run": QueryDatasetSpec(
        name="dividend_reinvestment_backtest_run",
        label="分红再投批次",
        description="分红再投入数据落地和回测计算批次记录。",
        model=DividendReinvestmentBacktestRun,
        columns=(
            col("id", "ID", 80),
            col("run_key", "批次键", 260),
            col("start_date", "开始日期", 132),
            col("end_date", "结束日期", 132),
            col("initial_amount", "初始投入", 120),
            col("cash_div_field", "分红口径", 120),
            col("status", "状态", 110),
            col("stock_count", "股票数", 100),
            col("summary_count", "汇总数", 100),
            col("started_at", "开始时间", 190),
            col("finished_at", "结束时间", 190),
            col("error_message", "错误", 260),
        ),
        keyword_fields=("run_key", "status", "cash_div_field", "error_message"),
        date_field="started_at",
        default_order=(("id", "desc"),),
    ),
